...
func _ready():
	_play_theme()
	var _useless = player.connect("player_died", Callable(self, "_player_died"))
	var _useless2 = player.connect("player_win", Callable(self, "_player_win"))
	dialog_handler.connect("end", Callable(self, "end_dialog"))
	var enemies = get_tree().get_nodes_in_group("enemy")
	for enemy in enemies:
		enemy.connect("died", Callable(self, "add_explosion"))
		if enemy is Bird:
			enemy.connect("bird_shot", Callable(self, "add_bird_shot"))

	var friends = get_tree().get_nodes_in_group("friend")
	for friend in friends:
		friend.connect("trigger_dialog", Callable(self, "open_dialog"))

	var checkpoints = get_tree().get_nodes_in_group("checkpoint")
	for checkpoint in checkpoints:
		checkpoint.connect("checkpoint_activated", Callable(self, "_activate_checkpoint"))
...
func _player_win():
	var _useless = get_tree().change_scene_to_file("res://End/End.tscn")

func add_explosion(position: Vector2):
	var d = enemy_death_particles.instantiate()
	d.position = position
	self.add_child(d)
	d.emitting = true
...
func add_bird_shot(shot):
	self.add_child(shot)
	shot.connect("shit_exploded", Callable(self, "add_bird_shot_explosion"))


func add_bird_shot_explosion(shot):
	var particles = shit_explosion_particles.instantiate()
